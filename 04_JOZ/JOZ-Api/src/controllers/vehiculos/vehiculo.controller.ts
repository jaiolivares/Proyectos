...
export class VehiculoController {
  ...
  public async crear(req: Request<{}, {}, VehiculoCreateRequestDto>, res: Response<Respuesta<VehiculoCreateResponseDto>>): Promise<Response<Respuesta<VehiculoCreateResponseDto>>> {
    try {
      ...
      return res.status(500).json(respuestaError<VehiculoCreateResponseDto>("ERROR CATCH: " + (err?.message ?? "error interno")));
    }
  }
  ...
  public async actualizar(req: Request<{ id: string }, {}, VehiculoUpdateRequestDto>, res: Response<Respuesta<VehiculoUpdateResponseDto>>): Promise<Response<Respuesta<VehiculoUpdateResponseDto>>> {
    try {
      ...
      if (err.message === "Vehículo no encontrado" || err.message === "IdMarcaModeloVehiculo no es válido") {
        return res.status(404).json(respuestaError<VehiculoUpdateResponseDto>(err.message));
      }

      errorMiddleware(err, req, res, () => {}, true);
      return res.status(500).json(respuestaError<VehiculoUpdateResponseDto>("ERROR CATCH: " + (err?.message ?? "error interno")));
    }
  }
