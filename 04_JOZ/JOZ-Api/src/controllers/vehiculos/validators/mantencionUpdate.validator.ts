export function ValidataEstructuraUpdateBody(body: any): { valid: boolean; errors?: string[] } {
  ...
  if (body.Fecha !== undefined && isNaN(new Date(body.Fecha).getTime())) {
    errors.push("Fecha inválida");
  }

  if (body.IdTaller !== undefined && (typeof body.IdTaller !== "number" || Number.isNaN(body.IdTaller))) {
    errors.push("IdTaller debe ser número");
  }

  if (body.Servicio !== undefined && (typeof body.Servicio !== "string" || body.Servicio.trim() === "")) {
    errors.push("Servicio inválido");
  }

  if (body.MontoTotal !== undefined && (typeof body.MontoTotal !== "number" || Number.isNaN(body.MontoTotal))) {
    errors.push("MontoTotal debe ser número");
  }

  if (body.Boleta !== undefined && body.Boleta !== null && typeof body.Boleta !== "string") {
    errors.push("Boleta inválida");
  }
