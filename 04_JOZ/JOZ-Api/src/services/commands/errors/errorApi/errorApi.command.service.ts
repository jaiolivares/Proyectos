...
export class ErrorApiCommandService {
  ...
  private obtenerNombreError(error: unknown): string {
    if (error instanceof Error && error.name) return this.recortar(error.name, 120);
    return 'Error';
  }

  private obtenerMensajeError(error: unknown): string {
    if (error instanceof Error && error.message) return this.recortar(error.message, 500);
    if (typeof error === 'string' && error.trim()) return this.recortar(error, 500);
  ...
  private obtenerStatusCode(error: unknown): number {
    if (typeof error === 'object' && error !== null) {
      const candidate = (error as { statusCode?: number; status?: number }).statusCode
